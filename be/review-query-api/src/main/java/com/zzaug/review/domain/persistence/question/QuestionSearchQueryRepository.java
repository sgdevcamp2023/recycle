...
public class QuestionSearchQueryRepository {
    ...
    public Page<QuestionQueryEntity> findAllByAuthorIdAndContentContainingAndIsDeletedIsFalse(
        ...
        NativeSearchQuery searchQuery = new NativeSearchQueryBuilder()
                ...
                .build();
        SearchHits<QuestionQueryEntity> searchHits = elasticsearchOperations
                .search(searchQuery, QuestionQueryEntity.class, IndexCoordinates.of("question"));

        return SearchHitSupport.searchPageFor(searchHits, searchQuery.getPageable()).map(SearchHit::getContent);
    }
}
