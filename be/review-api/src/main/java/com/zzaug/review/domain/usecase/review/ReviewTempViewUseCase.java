    ...
    public List<ReviewTempResponse> execute(ReviewTempViewUseCaseRequest request) {
        ...
        if (request.getTempId() != null) {
            ...
            return result.stream()
                    .map(ReviewTempResponseConverter::from)
                    .collect(Collectors.toList());
        } else {
            List<ReviewTempEntity> result = reviewTempRepository.findAllByAuthorIdAndQuestionId(request.getAuthorId(), request.getQuestionId());
            return result.stream()
                    .map(ReviewTempResponseConverter::from)
                    .collect(Collectors.toList());
        }
