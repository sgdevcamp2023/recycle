...
public class QuestionReqViewUseCase {
    ...
    public List<QuestionReqResponse> execute (QuestionReqViewUseCaseRequest request){
        ...
        List<QuestionRequestEntity> result = questionRequestRepository.findAllByReceiverId(request.getReceiverId());
        ...
        for(Long questionId : reqQuestionIds){
            QuestionEntity questionEntity = questionRepository.findById(questionId).orElseThrow(() -> new NoSuchElementException("No question found"));

            List<String> requesters = result.stream()
