...
const questionApi = {
  ...
  createQuestion: async ({ content }: QuestionProps) => {
    return await clientApi.common.post('/questions', { content });
  },

  // 질문 임시저장 [post]
  saveQuestionDraft: async ({ content, tId }: QuestionProps) => {
    return await clientApi.common.post('/questions', { content, t_id: tId });
  },

  // 질문 삭제 [delete]
  deleteQuestion: async ({ questionId: question_id }: QuestionProps) => {
    return await clientApi.common.delete(`/questions/${question_id}`);
  },

  // 리뷰를 달아준 리뷰어 목록 조회 [get]
  getQuestionReviwer: async ({ questionId: question_id }: QuestionProps) => {
    return await clientApi.common.get(`/me/questions/reviewers?${question_id}`);
  },
  ...
  getQuestionDrafts: async ({ tId: t_id }: QuestionProps) => {
    return await clientApi.query.get(`/me-query/questions/temp?${t_id}`);
  },

  // 리뷰 요청 목록 조회 [get]
  getReviewReqeusts: async () => {
    return await clientApi.query.get('/me-query/requests/reviews');
  },

  // 멤버의 질문 글 목록에서 검색 [get]
  searchQuestions: async ({ me, query, page, size }: SearchParamsProps) => {
    return await clientApi.query.get(`/question-query/search?${me}?${query}?${page}?${size}`);
  },

  // 질문 글 조회 [get]
  getQuestion: async ({ questionId: question_id }: QuestionProps) => {
    return await clientApi.query.get(`/question-query/${question_id}`);
  },
  ...
  getReviewOnQuestionDraft: async ({ questionId: question_id, tId: t_id }: QuestionProps) => {
    return await clientApi.query.get(`/question-query/${question_id}/reviews/temp?${t_id}`);
  },
};
